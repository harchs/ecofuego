ActiveAdmin.register Waste do
  ...
  filter :date_final_disposition
    ...
    f.inputs "Details" do
      ...
      f.input :weight, :input_html => {:style => "width:71px;"}, :label => "Weight (kg)"
      ...
      f.input :subdivision_id, :as => :select, :collection =>  Subdivision.free().order(:code).map {|s| [s.code, s.id]}, :label => "Storing in"
